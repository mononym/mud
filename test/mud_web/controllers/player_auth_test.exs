defmodule MudWeb.PlayerAuthTest do
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, MudWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "log_in_player/3" do
    test "stores the player token in the session", %{conn: conn, player: player} do
      ...
      assert Account.get_player_by_session_token(token)
    end
  ...
  describe "logout_player/1" do
    test "erases session and cookies", %{conn: conn, player: player} do
      player_token = Account.generate_player_session_token(player)
      ...
      assert redirected_to(conn) == "/"
      refute Account.get_player_by_session_token(player_token)
    end

    test "broadcasts to the given live_socket_id", %{conn: conn} do
      live_socket_id = "players_sessions:abcdef-token"
      MudWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "fetch_current_player/2" do
    ...
    test "does not authenticate if data is missing", %{conn: conn, player: player} do
      _ = Account.generate_player_session_token(player)

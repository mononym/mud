<script>
  ...
  function endSession() {
    endGameSession().then(() => $goto("../../auth/dashboard"));
  }
    ...
    <div class="flex items-center justify-between h-16">
      <div class="flex">
        <div class="flex items-baseline space-x-4">
          <a
            href={$url('/auth/play')}
            class="px-3 py-2 rounded-md text-sm font-medium {$isActive('/auth/play') ? 'text-white bg-gray-900' : 'text-gray-300 hover:text-white hover:bg-gray-700'}">Play</a>

          <a
            href={$url('/auth/build/world')}
            class="px-3 py-2 rounded-md text-sm font-medium {$isActive('/auth/build') ? 'text-white bg-gray-900' : 'text-gray-300 hover:text-white hover:bg-gray-700'}">Settings</a>
        </div>
      </div>

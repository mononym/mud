defmodule MudWeb.PlayerAuth do
  import Plug.Conn
  ...
  @max_age 60 * 60 * 24 * 60
  @remember_me_cookie "_mud_web_player_remember_me"
  ...
  def log_in_player(conn, player, params \\ %{}) do
    token = Account.generate_player_session_token(player)
  ...
  def log_out_player(conn) do
    ...
    player_token && Account.delete_session_token(player_token)

    if live_socket_id = get_session(conn, :live_socket_id) do
      MudWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def fetch_current_player(conn, _opts) do
    ...
    player = player_token && Account.get_player_by_session_token(player_token)
  ...
  """
  def fetch_current_player_live(session) do
    if player_token = Map.get(session, "player_token") do
      Account.get_player_by_session_token(player_token)
    else
      nil
    end
  end
